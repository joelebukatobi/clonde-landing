...
=============================================*/
  ...
  $(window).on('scroll', function () {
    ...
    scrollLink.each(function () {
      // 1. Check if the link actually has a hash (avoid href="#" or href="#!")
      var hash = this.hash;
      if (!hash || hash === '#' || hash === '#!') {
        return; // Skip this iteration
      }

      // 2. Try to select the element
      var section = $(hash);

      // 3. CRITICAL CHECK: Only proceed if the element exists on the CURRENT page
      if (section.length) {
        var sectionOffset = section.offset().top - 120;
